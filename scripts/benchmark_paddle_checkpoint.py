#!/usr/bin/env python3
"""Benchmark and rank PaddleOCR-VL full-SFT checkpoints on a gated split."""
...
from vlm_handwriting.paddle_vl import (
    load_paddle_vl_checkpoint,
    predict_one,
    validate_paddle_checkpoint,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Rank PaddleOCR-VL full-SFT checkpoints on validation before opening test"
        )
    )
    parser.add_argument("--checkpoint-path", type=Path, nargs="+", required=True)
    parser.add_argument("--manifest-root", type=Path, required=True)
    parser.add_argument("--raw-root", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, required=True)
    parser.add_argument("--config", type=Path, default=Path("configs/paddle/base.yaml"))
...
def benchmark_checkpoint(
    *,
    ...
    torch, processor, model, device, dtype = load_paddle_vl_checkpoint(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        result = {
            "model": "PaddleOCR-VL-1.6 full SFT",
        ...
        print(
            f"[{checkpoint_path.name} {index}/{len(rows)}] "
    ...
    metrics.update(
        {
            ...
            "dtype": str(dtype),
            "engine": str(config["model"]["engine"]),
            "prompt": str(config["model"]["prompt"]),
            "generation": config["generation"],
        }
    )
    write_csv(checkpoint_dir / "predictions.csv", predictions, PREDICTION_FIELDS)
...
def main() -> None:
    ...
    checkpoints = [validate_paddle_checkpoint(path) for path in args.checkpoint_path]
    enforce_frozen_test_single_adapter(split_name, len(checkpoints))
    checkpoint_names = [path.name for path in checkpoints]
    if len(set(checkpoint_names)) != len(checkpoint_names):
        raise ValueError(f"Checkpoint directory names must be unique: {checkpoint_names}")

    run_id = args.run_id or datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    run_dir = args.output_root / "paddleocr_vl" / "full_sft" / split_name / run_id
    ...
    for checkpoint_path in checkpoints:
        metrics = benchmark_checkpoint(
            checkpoint_path=checkpoint_path,
            rows=rows,
            split_name=split_name,
            raw_root=raw_root,
            config=config,
            checkpoint_dir=run_dir / checkpoint_path.name,
        )
    ...
    for rank, metrics in enumerate(
        ...
        print(
            f"{rank}. {metrics['checkpoint']}: CER={metrics['cer']:.6f} "
