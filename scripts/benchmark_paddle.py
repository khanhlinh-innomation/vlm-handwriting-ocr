#!/usr/bin/env python3
"""Run gated PaddleOCR-VL base benchmarks on the frozen line split."""
...
from vlm_handwriting.paddle_vl import load_paddle_vl_base, predict_one
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gated PaddleOCR-VL base benchmark: one -> smoke -> frozen test"
    )
...
def main() -> None:
    ...
    run_dir = args.output_root / "paddleocr_vl" / "base" / run_id
    prediction_path = run_dir / f"paddle_base_{artifact_tag}_predictions.csv"
    metrics_path = run_dir / f"paddle_base_{artifact_tag}_metrics.json"

    torch, processor, model, device, dtype = load_paddle_vl_base(config)
