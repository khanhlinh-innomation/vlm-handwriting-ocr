#!/usr/bin/env python3
"""Reload a saved PaddleOCR-VL full-SFT model and verify one validation inference."""
...
from vlm_handwriting.paddle_vl import load_paddle_vl_checkpoint, predict_one
...
def main() -> None:
    ...
    result = {
        "model": "PaddleOCR-VL-1.6 full-SFT smoke",
    ...
    write_csv(output_dir / "checkpoint_reload_prediction.csv", [result], list(result))
    write_json(
        output_dir / "checkpoint_reload_metrics.json",
        {
    ...
    print(f"Checkpoint reload verification passed: {output_dir}")
