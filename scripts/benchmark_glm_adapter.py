#!/usr/bin/env python3
"""Benchmark one or more GLM-OCR LoRA adapters on a gated frozen split."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rank GLM-OCR LoRA checkpoints on validation before opening test"
    )
...
def main() -> None:
    ...
    run_dir = args.output_root / "glm_ocr" / "lora" / split_name / run_id
    ...
    for adapter_path in adapters:
        ...
        summary_csv = [
            {field: row[field] for field in SUMMARY_FIELDS} for row in summary
        ]
