#!/usr/bin/env python3
"""Run gated TeleOCR base benchmarks on the frozen UIT-HWDB-line split."""
...
from vlm_handwriting.teleocr import load_teleocr_base, predict_one
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gated TeleOCR base benchmark: one -> smoke -> frozen test"
    )
...
def main() -> None:
    ...
    run_dir = args.output_root / "teleocr" / "base" / run_id
    prediction_path = run_dir / f"teleocr_base_{artifact_tag}_predictions.csv"
    metrics_path = run_dir / f"teleocr_base_{artifact_tag}_metrics.json"

    torch, processor, model, device, dtype = load_teleocr_base(config)
